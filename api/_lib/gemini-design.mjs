...
## 🗺️ Flow Diagram
สรุป User Flow ด้านบนเป็นแผนภาพ Mermaid flowchart เท่านั้น ห้ามมีข้อความอื่นนอกจาก code block นี้:

\`\`\`mermaid
flowchart LR
    A(["ผู้ใช้เริ่มต้น"]) --> B["ขั้นตอนการทำงาน"]
    B --> C{"จุดตัดสินใจ?"}
    C -->|ผ่าน| D["ขั้นตอนถัดไป"]
    C -->|ไม่ผ่าน| E["แจ้งเตือน/แก้ไข"]
    D --> F(["จบกระบวนการ"])

    classDef startEnd fill:#e6f4ff,stroke:#0958d9,color:#0958d9
    classDef process fill:#f0f7ff,stroke:#91caff,color:#0958d9
    classDef decision fill:#fff7e6,stroke:#faad14,color:#874d00
    classDef alert fill:#fff1f0,stroke:#ff4d4f,color:#a8071a
    class A,F startEnd
    class B,D process
    class C decision
    class E alert
...
กฎการเขียน Mermaid:
- ใช้ \`flowchart LR\` เสมอ (ซ้ายไปขวา)
- โหนดเริ่มต้น/จบ ใช้วงเล็บกลม \`(["ข้อความ"])\`, ขั้นตอนปกติใช้ \`["ข้อความ"]\`, จุดตัดสินใจใช้ \`{"ข้อความ"}\`
- ข้อความในโหนดต้องอยู่ในเครื่องหมายคำพูดคู่เสมอ ห้ามมีวงเล็บ, colon (:), หรือเครื่องหมายคำพูดซ้อนอยู่ข้างในข้อความ
- ห้ามใช้ตัวอักษร id ซ้ำกัน (A, B, C, ... ไล่ตามลำดับ)
- ใช้ classDef 4 แบบตามตัวอย่าง (startEnd, process, decision, alert) แล้ว assign ด้วย class ให้ครบทุกโหนด
- ถ้า Flow มีหลายฝ่ายเกี่ยวข้อง (เช่น ผู้ใช้ vs ระบบหลังบ้าน) ให้แบ่งเป็น subgraph ต่อฝ่าย เช่น \`subgraph S1["ฝั่งผู้ใช้"]\` ... \`end\`
- มีอย่างน้อย 4 โหนด และไม่เกิน 14 โหนด ให้ตรงกับ User Flow ด้านบน
